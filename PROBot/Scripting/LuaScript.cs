...
namespace PROBot.Scripting
{
    public class LuaScript : BaseScript
    {
        ...
        private void CreateLuaInstance()
        {
            ...
            _lua.Globals["isAllCaughtOn"] = new Func<string, bool>(IsAllCaughtOn);
            _lua.Globals["isTrainerBattles"] = new Func<bool>(IsTrainerBattles);
        ...
        //API: Return the state NPC battle
        private bool IsTrainerBattles()
        {
            return Bot.IsTrainerBattlesActive.IsEnabled;
        }

        //API: Activates npc trainers battling on sight.
	...
	private string[] GetPokemonTypeFromPC(int boxId, int boxPokemonId)
	{
        ...
        //API: Checks is all pokemon caught on a map or not
        private bool IsAllCaughtOn(string map)
        {
            Bot.Game.isRequestingForSpawnCheck = true;
            if(map == Bot.Game.MapName)
            {
                Bot.Game.isRequestingForCurrentSpawnCheck = true;
            }
            else
            {
                Bot.Game.isRequestingForCurrentSpawnCheck = false;
            }
            return Bot.Game.isAllPokemonCaughtOn(map);
        }
