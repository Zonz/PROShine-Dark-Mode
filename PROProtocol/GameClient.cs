    ...
    public class GameClient
    {
        ...
        private void UpdateScript()
        {
            if (IsScriptActive && !_dialogTimeout.IsActive)
            {
                ...
                {
                    //0-3 male - 4-7 female
                    if(CreatingCharacterMale == true)
                    {
                        SendCreateCharacter(Rand.Next(14), Rand.Next(28), Rand.Next(0, 3), Rand.Next(6), Rand.Next(5));
                    }
                    else if (CreatingCharacterFemale == true)
                    {
                        SendCreateCharacter(Rand.Next(14), Rand.Next(28), Rand.Next(4, 8), Rand.Next(6), Rand.Next(5));
                    }
                    IsScriptActive = false;
                    _dialogTimeout.Set();
                }
