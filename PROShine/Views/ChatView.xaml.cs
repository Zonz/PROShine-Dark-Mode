...
namespace PROShine
{
    public partial class ChatView : UserControl
    {
        ...
        private void AddChannelMessage(string channelName, string mode, string author, string message)
        {
            ...
            Regex linkRg = new Regex(regexPattern);
        ...
        private void AppendTextToRichTextBox(RichTextBox richTextBox,string author, string message)
        {
            ...
            if (linkMatch.Count >= 0) //If we find any pokemon link in Chat.
            {
                ...
                int position = message.LastIndexOf(']');
                string[] help = null;
                string textBeforeAlllink = "";
                string textAfterAllLink = "";
                if (!message.Contains("`"))
                {
                    help = Regex.Replace(message, regexPattern, "`").Split('`');//Trying to get messages before and after of pokemon links.
                    textBeforeAlllink =  Regex.Replace(message, regexPattern, "`").Split('`')[0];   //Message of before all pokemon links.
                    textAfterAllLink = help.Last(); //Message of after all pokemon links.
                }
                ...
                if (textAfterAllLink != string.Empty)
                {
                    ...
                    if (help.Length >= 0 && help != null)
                    {
                        var regex = new Regex(Regex.Escape(help.Last()));
                        textaftertcurrentlink = regex.Replace(textaftertcurrentlink, "", 1);
                    }
                    else
                    {
                        var regex = new Regex(Regex.Escape(textAfterAllLink));
                        textaftertcurrentlink = regex.Replace(textaftertcurrentlink, "", 1);
                    }
                }
                if (textBeforeAlllink != string.Empty)
                {
                    ...
                    if (help.Length >= 0 && help != null)
                    {
                        var regex = new Regex(Regex.Escape(help[0]));
                        textaftertcurrentlink = regex.Replace(textaftertcurrentlink, string.Empty, 1);
                    }
                    else
                    {
                        var regex = new Regex(Regex.Escape(textBeforeAlllink));
                        textaftertcurrentlink = regex.Replace(textaftertcurrentlink, "", 1);
                    }
                }
                //Links process
